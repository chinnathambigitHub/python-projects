...
def add_record():
    global connector, bk_name, bk_id, author_name, bk_status
    if bk_status.get() == 'Issued':
        card_id.set(issuer_card())
    else:
        card_id.set('N/A')

    if not bk_id.get() or not bk_name.get() or not author_name.get():
        mb.showerror('Missing fields', 'Please fill all the fields before adding a record.')
        return
...
def show_login():
    ...
    Label(login_window, text="Username", font=lbl_font).pack(pady=10)
    username_entry = Entry(login_window, font=entry_font)
    username_entry.pack(pady=5)

    Label(login_window, text="Password", font=lbl_font).pack(pady=10)
    password_entry = Entry(login_window, font=entry_font, show='*')
    ...
    def verify_credentials():
        ...
        if username == "admin" and password == "12345":
            mb.showinfo("Login Successful", "Welcome to the Library Management System")
            login_window.destroy()
            show_main_app()
        else:
            mb.showerror("Login Failed", "Invalid username or password")

    Button(login_window, text="Login", font=btn_font, bg=btn_hlb_bg, command=verify_credentials).pack(pady=20)
    login_window.mainloop()

def show_main_app():
    global bk_status, bk_name, bk_id, author_name, card_id, left_frame, tree, bk_id_entry, clear, btn_hlb_bg, lbl_font, entry_font, btn_font
